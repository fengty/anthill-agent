...
captcha-walls anonymous traffic. Anthill supports any of:
    ...
    TAVILY_API_KEY    — recommended, generous free tier, clean JSON
    SERPER_API_KEY    — Google results via serper.dev
    BRAVE_API_KEY     — Brave Search API

With none of those, web_search refuses with a clear message rather
than pretending to scrape. The CLI gives a one-liner to remediate.
"""
...
class WebSearchPlugin(Plugin):
    name = "web_search"
    description = "Search the web (requires TAVILY_API_KEY, SERPER_API_KEY, or BRAVE_API_KEY)."
    ...
    async def call(self, *, query: str, top_k: int = 5, **_: Any) -> PluginResult:
        ...
        return PluginResult(
            ...
            error=(
                ...
                "  export TAVILY_API_KEY=tvly-...   (recommended, free tier)\n"
