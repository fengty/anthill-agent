...
Spawns a small colony of identical agents (or a mix of providers), runs
several rounds of two different task types, and prints the pheromone map.

You should see one agent dominate "translate" and another dominate "explain"
purely from random cold-start choices followed by trail reinforcement.
...
TASKS = [
    ...
    ("explain", "Explain ant colony optimization in one sentence."),
    ("explain", "Explain swarm intelligence in one sentence."),
]


def print_trails(colony: Colony, title: str) -> None:
    ...
    model_by_id = {a.id: a.model for a in colony.agents}
    trails = sorted(colony.pheromones.trails(), key=lambda t: t.strength, reverse=True)
    for t in trails:
...
async def main() -> None:
    colony = Colony(name="demo", router_config=RouterConfig(exploration=0.10))

    # DeepSeek is required; MiniMax is optional.
    colony.spawn(count=2, model="deepseek-chat")
    if os.getenv("ANTHILL_MINIMAX_KEY") and os.getenv("ANTHILL_MINIMAX_GROUP"):
        colony.spawn(count=2, model="minimax")
        console.print("[dim]Spawned 2 DeepSeek + 2 MiniMax workers.[/dim]")
    else:
        colony.spawn(count=2, model="deepseek-chat")
        console.print("[dim]Spawned 4 DeepSeek workers (MiniMax keys not set).[/dim]")

    console.print()
    for i, (task_type, prompt) in enumerate(TASKS, start=1):
        result = await colony.run(task_type, prompt)
    ...
    console.print()
    print_trails(colony, "Pheromone trails after the run")
